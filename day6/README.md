# Part 1  
In this problem, let us not view the input as a computer would which is a n by m grid of characters, but rather as a human would where obviously understand where the numbers are and the operations are and solve it with that view.  
...
# Part 2  
In this problem, let us view the input as a computer would, a n b m grid of characters.  
...
For each number we fully take in a column, we store it into some indexed array called `nums`, so for example from right to left until the column of index `12` we will have in `nums`: `[4, 431, 623]`.  
