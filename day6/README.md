...
For column where we fully take in some number, we store that number into some indexed array called `nums`, so for example from right to left until the column of index `12` we will have in `nums`: `(4, 431, 623)`.  
