...
# Part 2:
This solution had to be a bit more specific because if it were to be without optimizations it'd take forever with bash so instead of counting all paths from one start to one end we count paths between specific pairs of nodes and then combine them:  
...
$$pathsof(svr->dac * dac->fft * fft->out) + pathsof(svr->fft * fft->dac * fft->out)$$  
