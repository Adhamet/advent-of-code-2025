# Part 1
Today's problem is fairly simple and simulative, all you need to do is loop on each cell and for each cell such that it is a roll: check in the eight adjacent cells `{(i-1,j-1), (i-1,j), (i-1,j+1), (i,j-1), (i,j+1), (i+1,j-1), (i+1,j), (i+1,j+1)}` for everyone of them such that they hold the value `@` then you increase the counter by `1`.  
